...
async function testGeminiAudio() {
  console.log('Starting Gemini audio modality test...');
  ...
  try {
    ...
    const inputTurns = 'Tell me a short joke about programming.';
    ...
    if (audioChunks.length > 0) {
      ...
      fs.writeFileSync('gemini_response.wav', wf.toBuffer());
      console.log('Audio saved to gemini_response.wav');
    } else {
...
async function main() {
  const success = await testGeminiAudio().catch(e => {
